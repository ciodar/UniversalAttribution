    ...
    def _update_section(section: 'Config', path: List[str], val: Any, resolved_section: 'Config') -> int:
        ...
        if key in resolved_section:
            ...
            try:
                ...
                raise KeyError(
                    f'The yaml key or command line argument "{key}" is likely not named correctly or value is of '
                    f'wrong data type. Error was occured when setting it to value "{val}".'
...
def parse_data_str(dataset_str: str):
    ...
    for token in tokens[1:]:
        ...
        kwargs[key] = value
    try:
        data_list = Config(config_filepath=f'./configs/data/{name.lower()}.yaml')[kwargs["split"]]
    except KeyError:
        print(f"Split {kwargs['split']} not found in {name.lower()}.yaml")
        data_list = Config(config_filepath=f'./configs/data/{name.lower()}.yaml')[name]
