...
def evaluate_multiclass(gt_labels, pred_labels):
    ...
    return {'recalls': recalls, 'recall': recall, 'f1': f1, 'accuracy': acc}
...
def compute_oscr(pred_k, pred_u, labels, out_curve=False):
    ...
    x1, x2 = np.max(pred_k, axis=1), np.max(pred_u, axis=1)  # known predictions, unknown predictions
    ...
    fpr95_pos = np.abs(np.asarray(FPR) - .05).argmin()
