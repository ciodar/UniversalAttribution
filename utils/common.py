...
def get_train_paths(data_list):
    data_path = os.path.abspath(data_list['data_path'])
    train_data_path = os.path.join(data_path, data_list['train_collection'], "annotations",
                                   data_list['train_collection'] + ".txt")
    val_data_path = os.path.join(data_path, data_list['val_collection'], "annotations",
                                 data_list['val_collection'] + ".txt")
...
def plot_OSCR_curve(results, save_path):
    ROC = sorted(zip(results['fpr'], results['ccr']), reverse=True)
    fpr, ccr = zip(*ROC)
    plt.figure(figsize=(8, 8))
    lw = 2
    plt.plot(fpr[1:], ccr[1:], color='darkorange',
             lw=lw, label='OSCR curve (area = %0.3f)' % results['oscr'])  ###假正率为横坐标，真正率为纵坐标做曲线
    plt.xlim([0.0, 1.0])
    plt.ylim([0.0, 1.05])
    plt.xlabel('False Positive Rate')
    plt.ylabel('Correct Classification Rate')
    plt.title('Open Set Classification Rate')
    plt.legend(loc="lower right")
    plt.savefig(save_path)
